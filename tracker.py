# tracker.py — Railway worker (finalized x-axis labels)
# Keeps: multi-webhook, dark theme, red Cylon, one combined PNG, replace-on-edit
# Fix: explicit level bracket labels shown (white, padded) on distribution subplot.
...
def _autolabel(ax, rects):
    for r in rects:
        ...
        ax.annotate(f"{int(h)}", xy=(r.get_x()+r.get_width()/2, h), xytext=(0,4),
                    textcoords="offset points", ha="center", va="bottom", fontsize=9, color="white")
...
def plot_combined(df_current: pd.DataFrame, region_label: str = "") -> io.BytesIO:
    ...
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(11, 9), gridspec_kw={"height_ratios":[1,1.1]})
    ...
    # Explicit x-axis labels (white, padded)
    ...
    ax1.set_xlabel("Level Range", fontsize=12); ax1.set_ylabel("Count", fontsize=12)
    ax1.legend()
    _autolabel(ax1, r1); _autolabel(ax1, r2)
    ax1.text(0.98, 0.92, f"Colonial: {sum(c_col)}   Cylon: {sum(c_cyl)}   Total: {len(df_current)}",
    ...
    if not df_hist.empty:
        ...
        ax2.set_xlabel("Timestamp (UTC)", fontsize=12); ax2.set_ylabel("Players Online", fontsize=12)
        ax2.legend()
        ...
        ax2.text(0.99, 0.03, _pct_leader_text(df_hist), transform=ax2.transAxes, ha="right", va="bottom",
                 fontsize=11, color="white", bbox=dict(facecolor="#1a1d22", edgecolor="#3a3f44", alpha=0.75, boxstyle="round,pad=0.35"))
    else:
        ax2.text(0.5,0.5,"No history yet to plot.", transform=ax2.transAxes, ha="center", va="center", fontsize=12, color="white")
        ax2.set_axis_off()

    plt.tight_layout(h_pad=2, rect=[0.03,0.03,0.99,0.99])
    buf = io.BytesIO(); fig.savefig(buf, format="png", dpi=300); plt.close(fig); buf.seek(0)
    return buf
