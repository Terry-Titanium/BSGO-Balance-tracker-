# tracker.py — No timestamps on time series; bracket labels parallel to x-axis
# Keeps: multi-webhook, dark theme, red Cylon, one combined PNG, replace-on-edit
# Changes: bottom chart has no timestamp axis; top chart uses annotation labels (parallel) for level brackets.
...
def plot_combined(df_current: pd.DataFrame, region_label: str = "") -> io.BytesIO:
    ...
    ax1.set_xticks([])  # remove regular ticks/labels
    for i, label in enumerate(categories):
        ax1.text(i, -0.12, label,
                 transform=ax1.get_xaxis_transform(),  # anchor to axis coords vertically
                 ha="center", va="top",
