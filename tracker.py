...
# Extra: rotate labels, larger font, added bottom margin, save without bbox_inches to avoid clipping.
...
def plot_combined(df_current: pd.DataFrame, region_label: str = "") -> io.BytesIO:
    ...
    # Explicit x-axis labels (white, rotated, padded, forced visible)
    ax1.set_xticks(x)
    ax1.set_xticklabels(categories, fontsize=12, rotation=25, ha="right")
    for tick in ax1.get_xticklabels():
        tick.set_color("white")
        tick.set_visible(True)
    ax1.tick_params(axis="x", pad=12, labelbottom=True)
    ...
    fig.tight_layout(h_pad=2.6, rect=[0.04,0.12,0.995,0.995])
    buf = io.BytesIO()
