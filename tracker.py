...
# Features:
# - Multiple webhooks (comma-list or JSON)
# - Dark theme
# - Cylon = red (#ff0000)
# - Distribution bar labels with level brackets on x-axis
# - Combined image (distribution + time series)
# - Edits previous webhook message and REPLACES the attachment
...
def plot_combined(df_current: pd.DataFrame, region_label: str = "") -> io.BytesIO:
    ...
    # restore x-axis level bracket labels
...
def send_to_discord(webhook_url: str, combined_png: io.BytesIO, summary_text: str) -> None:
    ...
    if last_id:
        data["payload_json"] = json.dumps({"attachments": []})
